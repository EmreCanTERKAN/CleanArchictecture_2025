...
namespace CleanArchictecture.Application.Auth;
...
internal sealed class LoginCommandHandler(UserManager<AppUser> userManager, SignInManager<AppUser> signInManager, IJwtProvider jwtProvider) : IRequestHandler<LoginCommand, Result<LoginCommandResponse>>
{
    ...
    {
        AppUser? user = await userManager.Users.FirstOrDefaultAsync(p => p.Email == request.UserNameOrEmail || p.UserName == request.UserNameOrEmail);
        ...
        if (signInResult.IsLockedOut)
        {
            ...
            if (timeSpan is not null)
            {
                ...
                return (500, "Kullanıcı 5 kez yanlış şifre girdiği için 5 dakika süreyle bloke edilmiştir.");
        }
